                ...
                <div class="bg-white rounded-lg shadow-sm p-8">
                    <form method="POST" action="{{ route('risalah.store', [$jadwal->jadwal_id, $jenis_risalah]) }}">
                        ...
                        <div>
                            ...
                            <textarea name="pokok_masalah" class="form-input w-full rounded border-gray-300 focus:ring-blue-500">{{ old('pokok_masalah') }}</textarea>
                        </div>
                        <div>
                            <label class="block font-medium mb-1">Pendapat Pekerja/Buruh/SP/SB</label>
                            <textarea name="pendapat_pekerja" class="form-input w-full rounded border-gray-300 focus:ring-blue-500">{{ old('pendapat_pekerja') }}</textarea>
                        </div>
                        <div>
                            <label class="block font-medium mb-1">Pendapat Pengusaha</label>
                            <textarea name="pendapat_pengusaha" class="form-input w-full rounded border-gray-300 focus:ring-blue-500">{{ old('pendapat_pengusaha') }}</textarea>
                        </div>
                        <div class="mt-6 space-y-4">
                            @if ($jenis_risalah === 'klarifikasi')
                                <div>
                                    <label class="block font-medium mb-1">Arahan Mediator</label>
                                    <textarea name="arahan_mediator" class="form-input w-full rounded border-gray-300 focus:ring-blue-500">{{ old('arahan_mediator') }}</textarea>
                                </div>
                                <div>
                            ...
                            @if ($jenis_risalah === 'penyelesaian')
                                <div>
                                    ...
                                    <textarea name="kesimpulan_penyelesaian" class="form-input w-full rounded border-gray-300 focus:ring-blue-500">{{ old('kesimpulan_penyelesaian') }}</textarea>
                                </div>
                            @endif
                        </div>
